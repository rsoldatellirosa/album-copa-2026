    ...
    <header className="sticky top-0 z-20 bg-cream/85 backdrop-blur border-b border-black/5">
      <div className="max-w-4xl mx-auto px-4 h-14 flex items-center justify-between gap-3">
        <Link href="/" className="flex items-center gap-2 shrink-0">
          <span className="w-7 h-7 rounded-lg bg-mint flex items-center justify-center text-sm">⚽</span>
        ...
        <nav className="flex items-center gap-1 text-sm">
          ...
          {unlocked ? (
            <button
              onClick={lock}
              className="ml-1 px-2.5 py-1.5 rounded-full text-ink hover:bg-black/5"
            ...
            <button
              ...
              className="ml-1 px-2.5 py-1.5 rounded-full text-faint hover:bg-black/5"
      ...
      {asking && (
        <div className="fixed inset-0 z-30 flex items-center justify-center bg-black/30 p-4" onClick={() => setAsking(false)}>
          <div className="bg-paper rounded-2xl p-5 w-full max-w-xs" onClick={(e) => e.stopPropagation()}>
            ...
            <input
              ...
              autoFocus
              className="w-full bg-cream border border-black/10 rounded-xl px-3 py-2 outline-none focus:ring-2 focus:ring-mint"
              placeholder="PIN"
            />
            {error && <p className="text-coral text-sm mt-2">{error}</p>}
            <div className="flex gap-2 mt-4">
              <button onClick={() => setAsking(false)} className="flex-1 py-2 rounded-xl border border-black/10 text-ink/60">
                Cancelar
              </button>
              <button onClick={submit} className="flex-1 py-2 rounded-xl bg-mint-deep text-white font-medium">
                Entrar
              </button>
            </div>
          </div>
